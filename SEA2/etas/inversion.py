...
class ETASParameterCalculation:
    def __init__(self, metadata: dict):
        ...
        self.auxiliary_start = pd.to_datetime(metadata["auxiliary_start"],dayfirst=True)
        self.timewindow_start = pd.to_datetime(metadata["timewindow_start"],dayfirst=True)
        self.timewindow_end = pd.to_datetime(metadata["timewindow_end"],dayfirst=True)
        ...
        if not isinstance(self.catalog, pd.DataFrame):
            ...
            self.catalog["time"] = pd.to_datetime(
                self.catalog["time"], format="%d-%m-%Y %H:%M",dayfirst=True
            )
    ...
    def load_calculation(cls, metadata: dict):
        ...
        obj.auxiliary_start = pd.to_datetime(metadata["auxiliary_start"],dayfirst=True)
        obj.timewindow_start = pd.to_datetime(metadata["timewindow_start"],dayfirst=True)
        obj.timewindow_end = pd.to_datetime(metadata["timewindow_end"],dayfirst=True)
        ...
        obj.catalog["time"] = pd.to_datetime(obj.catalog["time"], format="%d-%m-%Y %H:%M",dayfirst=True)
