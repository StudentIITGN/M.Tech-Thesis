...
def productivity_plot(
    ...
    counts, _ = np.histogram(p_mat["source_magnitude"], bins=magnitude_bins, weights=p_mat["Pij"] * p_mat["zeta_plus_1"])
    # Calculate the number of events in each bin
    how_many, _ = np.histogram(catalog["magnitude"], magnitude_bins)

    # Safely scale counts by the number of events in each bin
    print("Counts:", counts)
    print("How many:", how_many)

                    # Safely scale counts by the number of events in each bin
    counts_scaled = np.zeros_like(counts, dtype=float)
    non_zero_bins = how_many != 0

                    # Perform the scaling only for bins where how_many is not zero
    counts_scaled[non_zero_bins] = counts[non_zero_bins] / how_many[non_zero_bins]

                # Additional debugging to check the scaled counts
    print("Counts scaled:", counts_scaled)
    plt.figure()
